...
## Design agent: brief to proof to delivered order (2026-09-22)
## Status per part
- Agent built (`design-agent/design-agent.js`, `design-agent/design-specs.json`): DONE
  evidence: `node --check design-agent/design-agent.js` -> exit 0
- Test suite: DONE
  evidence: `node design-agent/test-design-agent.js` -> `RESULT: 34 passed, 0 failed`
- Brief to proof: DONE
  evidence: `node design-agent/design-agent.js --brief design-agent/jobs/example-brief.json --out design-agent/out --render` -> `AWAITING_APPROVAL`, variants A badge/forest, B stack/sunset, C minimal/ocean; `proof.html` and `proof.png` written; `order.json exists before approval? False`
- Approval to delivery: DONE
  evidence: `--approve A --by "Maya"` -> `SM-BR-2001-R1 -> PROD-BR-2001-R1 -> SHP-BR-2001-R1`, state `DELIVERED`, `delivery.txt` written
- Preflight handoff: DONE
  evidence: `audit.log` line `{"event":"preflight","ok":true,"decision":"AUTO_APPROVED","reasons":"No blocking issues found"}` before `ORDER_CREATED`
- Model-backed concept provider: PARTIAL (implemented, gated)
  evidence: remote provider without a key returns `model provider unavailable (no API key found ...); used local concepts`; the HTTP call itself is UNVERIFIED because no key was available

## What broke and how I fixed it
- The rendered proof showed the customer brand text clipped outside the artboard on all three variants. I caught it by reading `proof.png`, not by reading the code. Fix: added `fitLines()`/`textBlock()`, which measure against an estimated glyph width, wrap onto up to 3 lines and shrink the font to fit; added two regression tests. Re-render confirms text inside the circle.
- The design agent suite failed on its second run because the temp folder was reused and the first run's `DELIVERED` state short-circuited regeneration. Fix: clear the temp folder at the start of the suite. This was a test bug, not an agent bug.

## Claims ledger (design agent)
- No order without approval: proved by `NO order exists before approval` and `fulfil refuses with no approval on record`.
- An approval binds to the exact artwork: proved by `order binds to the artwork hash` and `artwork changed after approval invalidates it`.
- The pipeline only advances in order: proved by `state order is APPROVED -> ORDER_CREATED -> IN_PRODUCTION -> SHIPPED -> DELIVERED`.
- A bad file cannot be ordered: proved by `corrupt artwork is blocked by the preflight gate`.
- Text fits the print area: proved by `long brand text is scaled to fit inside the artboard`.
- UNVERIFIED: the remote model call, because no API key was available; it falls back to local and records the reason.
- UNVERIFIED: real Sticker Mule artwork, because none was used; variants are generated by the local renderer.

## What I would tell the next person
- Change palettes, layouts, print DPI, turnaround and revision cap in `design-agent/design-specs.json`.
- The approval is stored with the artwork hash on purpose: if a designer edits a file after approval, the next run must invalidate the approval rather than ship a different file than the customer saw.
